...
public class AVLTreeAddressTable {
	...
	private BinaryTreeNode rotateRight(BinaryTreeNode x) {
	...
	private BinaryTreeNode rotateLeft(BinaryTreeNode x) {
	...
	private BinaryTreeNode rotateLeftThenRight(BinaryTreeNode x) {
		x.left = rotateLeft(x.left);
		x = rotateRight(x);
		return x;

	}
	private BinaryTreeNode rotateRightThenLeft(BinaryTreeNode x) {
		x.right = rotateRight(x.right);
		x = rotateLeft(x);
		return x;
	}
	
	private BinaryTreeNode rebalance(BinaryTreeNode x) {
		...
		else if(height(x.right)>1+height(x.left)) {
			if(height(x.right.right)<height(x.right.left)) {
				x= rotateRightThenLeft(x);
			}
			else {
				x = rotateLeft(x);
			}
	...
	public static void main(String args[]) throws FileNotFoundException {
		...
		String tmpValue="";
		int rootSize;
		AVLTreeAddressTable addressTable = new AVLTreeAddressTable();
